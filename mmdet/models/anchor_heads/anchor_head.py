...
class AnchorHead(nn.Module):
    ...
    def loss(self,
        ...
        print(gt_bboxes)
        if gt_labels==None:
            gt_labels=torch.tensor((),dtype = torch.uint8)
            gt_labels.new_ones(len(gt_bboxes)) 
        print(gt_labels)
        ...
        if self.use_kl_loss:
            ...
            for i in range(len(mus)):
                kld_losses.append(self.KLD_loss(mus[i],logvars[i]))
        if self.use_kl_loss:
            return dict(loss_cls=losses_cls, loss_reg=losses_reg,loss_kld=kld_losses)
